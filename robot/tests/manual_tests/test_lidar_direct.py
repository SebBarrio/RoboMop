...
def direct_scan(port: str, duration: float = 2.0, output_dir: str = "logs"):
    ...
    try:
        # Open serial port
        ser = serial.Serial(port=port, baudrate=1000000, timeout=1.0)
        ...
        try:
            ...
            print(
                f"✓ Model: {model}, Firmware: {firmware_major}.{firmware_minor}, "
                f"Hardware: {hardware}"
            )
        ...
        scatter = ax1.scatter(
            np.deg2rad(angles), distances, c=qualities, cmap="viridis", alpha=0.75, s=2
        )
        ax1.set_title(f"Direct RPLidar Scan\n{len(scan_data)} measurements", pad=20, fontsize=12)
