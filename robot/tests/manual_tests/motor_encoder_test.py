            ...
            if not result['responding_encoders']:
                ...
                if observed_enc == expected_enc:
                    if "FORWARD" in direction:
                        ...
                        status = "✗ REVERSED"
                        issues.append(f"Motor {motor_idx}: Direction reversed (encoder counts negative)")
                else:
                    status = "✗ WRONG ENC"
                    issues.append(f"Motor {motor_idx}: Wrong encoder (expected {expected_enc}, got {observed_enc})")
            
            print(f"{motor_idx:<8} {expected_enc:<12} {str(observed_enc):<12} {direction:<15} {status:<15}")
        ...
        for result in self.results:
            ...
            if result['responding_encoders'] and len(result['responding_encoders']) == 1:
                ...
                if delta < 0:
                    ...
                    code_changes.append(f"  - Motor {motor_idx}: Swap channels ({fwd_ch}, {rev_ch}) → ({rev_ch}, {fwd_ch})")
