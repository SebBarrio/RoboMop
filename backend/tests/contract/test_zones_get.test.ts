...
describe("GET /api/v1/maps/:mapId/zones", () => {
  it("returns restricted zones for a map", async () => {
    ...
    const createZoneResponse = await request(app)
    ...
    if (Array.isArray(response.body)) {
      response.body.forEach((zone: unknown) => expectRestrictedZoneSchema(zone));
    }
  });

  it("returns 404 when the map does not exist", async () => {
    const response = await request(app)
      .get(`/api/v1/maps/${randomUUID()}/zones`)
      .set("Accept", "application/json");
